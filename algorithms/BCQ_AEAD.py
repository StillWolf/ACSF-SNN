...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action, phi=0.05, T=32, tau=2.0, alpha=2.0, v_threshold=1.0,
        ...
        self.Decoder = nn.Linear(T * 256, 256)
        self.l1 = nn.Sequential(layer.SeqToANNContainer(nn.Linear(state_dim + action_dim, 256)),
                                neuron.MultiStepLIFNode(tau, v_threshold=v_threshold, v_reset=v_reset,
                                                        surrogate_function=surrogate_func))
        self.l2 = nn.Sequential(layer.SeqToANNContainer(nn.Linear(256, 256)),
                                neuron.MultiStepLIFNode(tau, v_threshold=v_threshold, v_reset=v_reset,
                                                        surrogate_function=surrogate_func))
    ...
    def forward(self, state, action):
        ...
        input = input.view(input.shape[0], self.T, -1)
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, T=32, tau=2.0, alpha=2.0, v_threshold=1.0, v_reset=0.0):
        ...
        self.l4 = nn.Sequential(layer.SeqToANNContainer(nn.Linear(state_dim + action_dim, 256)),
                                neuron.MultiStepLIFNode(tau, v_threshold=v_threshold, v_reset=v_reset,
                                                        surrogate_function=surrogate_func))
        self.l5 = nn.Sequential(layer.SeqToANNContainer(nn.Linear(256, 256)),
                                neuron.MultiStepLIFNode(tau, v_threshold=v_threshold, v_reset=v_reset,
                                                        surrogate_function=surrogate_func))
