...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action, phi=0.05, T=4):
        ...
        self.l1 = nn.Linear((state_dim + action_dim) * T, 400)
        self.l2 = nn.Linear(400, 300)
        self.l3 = nn.Linear(300, action_dim * T)
        self.decoder = nn.Linear(action_dim * T, action_dim)

        self.max_action = max_action
        self.phi = phi

    def forward(self, state, action):
        a = F.relu(self.l1(self.encoder(torch.cat([state, action], 1))))
        a = F.relu(self.l2(a))
        a = self.phi * self.max_action * torch.tanh(self.decoder(self.l3(a)))
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, T=4):
        ...
        self.encoder2 = nn.Linear(state_dim + action_dim, (state_dim + action_dim) * T)
        self.decoder1 = nn.Linear(T, 1)
        self.decoder2 = nn.Linear(T, 1)
        self.l1 = nn.Linear((state_dim + action_dim) * T, 400)
        self.l2 = nn.Linear(400, 300)
        self.l3 = nn.Linear(300, T)

        self.l4 = nn.Linear((state_dim + action_dim) * T, 400)
        self.l5 = nn.Linear(400, 300)
        self.l6 = nn.Linear(300, T)

    def forward(self, state, action):
        q1 = F.relu(self.l1(self.encoder1(torch.cat([state, action], 1))))
        q1 = F.relu(self.l2(q1))
        q1 = self.decoder1(self.l3(q1))

        q2 = F.relu(self.l4(self.encoder2(torch.cat([state, action], 1))))
        q2 = F.relu(self.l5(q2))
        q2 = self.decoder2(self.l6(q2))
        return q1, q2
